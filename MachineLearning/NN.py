...
class NeuralNetwork:
    ...
    *** DISCLAIMER ***:
    ...
    def trian(self, data, all_y_trues):
        alpha = 0.1
        epochs = 1000
        M = len(data)
        ...
        for epoch in range(epochs):
            for x, y_true in zip(data, all_y_trues):
                sum_h1 = np.dot([self.w1, self.w2], x)+self.b1
                h1 = sigmoid(sum_h1)

                sum_h2 = np.dot([self.w3, self.w4], x) + self.b2
                h2 = sigmoid(sum_h2)

                sum_o1 = np.dot([self.w5, self.w6], [h1, h2]) + self.b3
                o1 = sigmoid(sum_o1)
...
network.trian(data, all_y_trues)
